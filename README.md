...
Azure Storage включает в себя множество подсервисов: 
  ...
  * Azure Container Storage: Служба управления томами, развертывания и оркестровки, созданная специально для контейнеров.
  ...
  Сервис предоставляет доступ к различным безопасным и надежным вычислительным ресурсам:
  
  * Равертывание разнообразных приложений. Запуск приложений от SQL Server, Oracle и SAP до программного обеспечения с открытым исходным кодом и высокопроизводительных вычислений.

  * Максимизация экономии. Оптимизация расходов с помощью резервирования Azure, точечных виртуальных машин Azure, экономичного плана Azure для вычислений и преимущества гибридного использования Azure.

  * Быстрое и надежное масштабирование.

  * Миграция и расширение приложений.
  ...
  Коннекторы BizTalk Services помогают мостам отправлять и получать данные, включая двусторонние мосты, обеспечивающие взаимодействие на основе HTTP-запросов/ответов. В Logic Apps используется та же терминология и имеются сотни коннекторов, которые служат той же цели, подключаясь к широкому спектру технологий и служб. Например, коннекторы доступны для облачных SaaS и PaaS сервисов, таких как OneDrive, Office365, Dynamics CRM и других, а также для локальных систем через On-Premises Data Gateway, который заменяет BizTalk Adapter Service для BizTalk Services. По умолчанию каждый мост имеет конечную точку HTTP, которая настроена со свойствами Runtime Address и Relative Address для моста. 

  В BizTalk Services мост аналогичен processing pipeline. Мост может принимать данные, полученные от коннектора, выполнять с ними определенную работу и отправлять результаты в другую систему. Logic Apps делает то же самое, поддерживая те же шаблоны взаимодействия на основе конвейера, что и BizTalk Services, а также предоставляя другие шаблоны интеграции. Мост XML Request-Reply в BizTalk Services известен как конвейер VETER, который состоит из этапов, выполняющих следующие задачи:
  * (V) Validate
  * (E) Enrich
  * (T) Transform
  * (E) Enrich
  * (R) Route
  ...
  Azure DevOps предоставляет интегрированные функции, которые можно получить через веб-браузер или клиент интегрированной среды разработки. Можно использовать все службы, включенные в Azure DevOps, или выбрать только то, что необходимо дополнить существующие рабочие процессы:
  * Azure Boards - Предоставляет набор средств Agile для поддержки планирования и отслеживания работы, дефектов кода и проблем с помощью методов Kanban и Scrum.
  * Azure Repos - Предоставляет репозитории Git или система управления версиями Team Foundation (TFVC) для управления версиями кода.
  * Azure Pipelines - Предоставляет службы сборки и выпуска для поддержки непрерывной интеграции и доставки приложений.
  * Azure Test Plans - Предоставляет несколько средств для тестирования приложений, включая ручное или исследовательское тестирование и непрерывное тестирование.
  * Azure Artifacts - Позволяет командам совместно использовать такие пакеты, как Maven, npm, NuGet и многое другое из общедоступных и частных источников, а также интегрировать общий доступ к пакетам в конвейеры. 
  ...
  Azure Monitor может отслеживать следубщие типы ресурсов:
  * Приложения
  * Виртуальные машины
  ...
  * Пользовательские источники, использующие API для получения данных в Azure Monitor
  ...
  Azure Data and AI включает в себя множество сервисов:

  * Azure AI Speach - Перенос облачного поиска на основе искусственного интеллекта в мобильные и веб-приложения
  * Azure OpenAI - Выполнение широкого спектра задач на естественном языке
  * Bot Service - Создание ботов и подключение их к различным каналам.
  * Content Safety - Сервис искусственного интеллекта для обнаружения нежелательного контента
  * Custom Vision - Настройка распознавания изображений в соответствии с требованиями бизнеса
  * Document Intelligence - Превращение документов в пригодные для использования данные с меньшими затратами времени и средств.
  * Face - Обнаружение и идентификация людей и эмоций на изображениях
  * Immersive Reader - Помощь пользователям читать и понимать текст
  * Language - Создание приложений с лучшими в отрасли возможностями понимания естественного языка
  * Speech - Речь в текст, текст в речь, перевод и распознавание диктора
  * Translator - Перевод более чем на 100 языков и диалектов
  * Video Indexer - Извлечение полезных сведений из видео
  * Vision - Анализ содержимого изображений и видео
...
к масштабированию, уменьшить затраты на IT, открыть данные для мониторинга состояния бизнеса и сократить время вывода новых решений на рынок
  ...
  * Для условной переадресации запросов - используя правила маршрутизации можно перенаправлять запросы в соответствии с префиксами имен объектов или HTTP-кодами ответов. Вы можете перенаправить на другую веб-страницу запрос к удаленному объекту или перенаправить запросы, возвращающие ошибку.
  ...
  Бакет — это контейнер для хранения объектов в хранилище. Для работы хостинга необходим публичный доступ к бакету.
   ...
   11. **Combyx** - это многофункциональная платформа для управления корпоративными данными о клиентах, создания Golden Record, внедрения Data Governance и Master Data Management. Сервис включает в себя интеграцию источников данных, загрузку и контроль выходных данных, очистку и дедубликацию данных, создание Customer Golden Record, создание витрины и доступ к данным.

  Customer Golden Record относится к "single source of truth" или "single customer view", которое состоит из единой, надежной версии данных, содержащей всю необходимую информацию о клиенте.

  Компания подключает Data Factory ко всем системам клиента — CRM, call-центру, BI, CDP, Ecom, App — используя только свои IT-ресурсы; Настраивает нужные фильтры и правила объединения данных в одном месте, правила сбора, обработки и возврата нормализованных данных в системы клиента; Создает необходимые витрины данных и наглядные формы отчетности. Инструктирует всех пользователей и организует поддержку.


## Вывод

В ходе выполнения лабораторной работы были проанализированы некоторые Azure сервисы, найдены их полные или частичные аналоги в России. Информация о сервисах была взята из официальных документаций Microsoft и российских сервисов. Была заполнена таблица, в которой получилось распределить сервисы Azure ни типы и подтипы, подобрать типы использования. Так как для всех сервисов были найдены аналоги, можно сделать вывод, что миграция возможна, несмотря на то, что для некоторых сервисов функционал аналогов значительно уже, чем сервисов Azure. Например, сервис Yandex Object Storage, являясь универсальным хранилищем, не полностью предоставляет функционал Azure Storage, который включает в себя множество сервисов, начиная с табличных хранилищ (Azure Table) и заканчивая службой управления томами для контейнеров (Azure Container Storage). Тем не менее, большинство возможностей Azure представлены у отечественных провайдеров.
